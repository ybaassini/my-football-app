...
describe('ApiHandlerService', () => {
  ...
  const request: HttpRequest<any> = new HttpRequest('POST', '/api/sessions', null);
  const responseSuccess: HttpResponse<any> = new HttpResponse({
    body: {},
    status: 200,
    url: 'http://localhost:3000/api/sessions'
  });
  const responseError: HttpResponse<any> = new HttpResponse({
    body: {},
    status: 404,
    url: 'http://localhost:3000/api/sessions'
  });
